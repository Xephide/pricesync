...
exports.handler = async function(event) {
  ...
  async function tryUpdate(url) {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        r = await fetch(url, { method: 'POST', headers: { 'Api-Key': token, 'Content-Type': 'application/json' }, body: JSON.stringify({ offers }) });
        rawText = await r.text();
        if (r.status === 420 || r.status === 429) { await sleep(2500 * (attempt + 1)); continue; }
  ...
  const errMsg = parsed?.errors?.[0]?.message || parsed?.error || `HTTP ${res.status}: ${String(res.text).slice(0, 300)}`;
